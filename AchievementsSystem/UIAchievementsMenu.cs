...
namespace CookieClicker.AchievementsSystem
{
	public class UIAchievementsMenu : UIState
	{
		...
		private readonly List<UIAchievementListItem> _achievementElements = new List<UIAchievementListItem>();
		...
		public void InitializePage()
		{
            ...
            UITextPanel<LocalizedText> uITextPanel = new UITextPanel<LocalizedText>(Language.GetText("UI.Achievements"), 1f, large: true) { HAlign = 0.5f };
			...
			List<Achievement> list = CookieClicker.Achievements.CreateAchievementsList();
			...
			Texture2D texture = ModContent.GetTexture("CookieClicker/UI/Categories");
			for (int j = 0; j < 5; j++)
			{
		...
		public void GotoAchievement(Achievement achievement)
		{
			_achievementsList.Goto(delegate (UIElement element) {
                return element is UIAchievementListItem uIAchievementListItem && uIAchievementListItem.GetAchievement() == achievement;
            });
		}
		...
		private void FadedMouseOver(UIMouseEvent evt, UIElement listeningElement)
		{
			...
			((UIPanel)evt.Target).BorderColor = Main.OurFavoriteColor;
		}
